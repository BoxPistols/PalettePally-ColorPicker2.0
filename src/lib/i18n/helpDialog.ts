// HelpDialog のユーザー向け文言（ハードコード文字列の定数化）
export const helpDialog = {
  title: '使い方ガイド',
  subtitle: 'Palette Pally — MUI Color Palette Generator',
  intro:
  ...
  step2Desc:
    '各色に対して Material Design 3 ベースの 5段階パレット (main / dark / light / lighter / contrastText) が light / dark 両モードで生成されます。',
  ...
  step3Desc:
    '各カードの右上ペンアイコン をクリックすると編集 Dialog が開きます。個別の色をカラーピッカー / HEX で編集できます。',
  ...
  step4Desc:
    'Utility Tokens セクションの「+ Add Token Group」から独自のトークンカテゴリを追加できます。',
  ...
  step5Desc:
    'ヘッダーの「Example」リンクから、生成したパレットを使った MUI コンポーネントのプレビューを確認できます。',
  ...
  step7Desc:
    '「Figma」ボタンから Personal Access Token とファイル URL を入力すると、生成パレットを Figma Variable Collection として push できます。',
